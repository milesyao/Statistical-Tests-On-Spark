 ...
 */
@DeveloperApi
class PatchedMultivariateOnlineSummarizer extends MultivariateOnlineSummarizer with Serializable {
  ...
  private var nnn: Array[Double] = _
   ...
   * @return This MultivariateOnlineSummarizer object.
   */
  def merge(other: PatchedMultivariateOnlineSummarizer): this.type = {
    if (this.totalCnt != 0 && other.totalCnt != 0) {
      ...
      while (i < n) {
        ...
        val thisNnn = nnn(i)
        val otherNnn = other.nnn(i)
        val totalNnz = thisNnz + otherNnz
        val totalNnn = thisNnn + otherNnn
        if (totalNnz != 0.0) {
  ...
  override def mean: Vector = {
    ...
    var i = 0
    while (i < n) {
      realMean(i) = currMean(i) * (nnz(i) / nnn(i))
      i += 1
    }
  ...
  override def variance: Vector = {
    ...
    if (denominator > 0.0) {
      ...
      while (i < currM2n.size) {
        realVariance(i) =
          currM2n(i) + deltaMean(i) * deltaMean(i) * nnz(i) * (totalCnt - nnz(i)) / nnn(i)
  ...
  override def max: Vector = {
    ...
    var i = 0
    while (i < n) {
      if ((nnz(i) < nnn(i)) && (currMax(i) < 0.0)) currMax(i) = 0.0
      i += 1
    }
  ...
  override def min: Vector = {
    ...
    var i = 0
    while (i < n) {
      if ((nnz(i) < nnn(i)) && (currMin(i) > 0.0)) currMin(i) = 0.0
      i += 1
    }
