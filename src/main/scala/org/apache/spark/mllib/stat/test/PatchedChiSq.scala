...
import breeze.linalg.{DenseMatrix => BDM, min}
 ...
 */
object PatchedChiSq extends Logging{
  ...
  def chiSquaredFeatures(data: RDD[LabeledPoint],
      methodName: String = PEARSON.name): Array[PatchedChiSqTestResult] = {
    val maxCategories = 10000
    val numCols = data.first().features.size
    val results = new Array[PatchedChiSqTestResult](numCols)
    ...
    while (batch * batchSize < numCols) {
      ...
      }.countByValue()//This function derives a local Map

   /*   val EntryCheck = pairCounts.toArray.groupBy(_._1._1).map(i => {
        val col = i._1
        val good_entry = i._2.filter(j => j._2 > 5).length
        val total_entry = i._2.length
        val good_ratio  = good_entry / total_entry
        if (good_ratio < 0.8) {
          logWarning(s"Chi-squared independence dest for column $col may not be accurate due to ")
        }
      })*/
  ...
  def ChiSquared_TwoSample(observed: Array[Double],
    ...
    val statistic = obsArr.zip(expArr).foldLeft(0.0) { case (stat, (obs, exp)) =>
      if (exp == 0.0) {
        if (obs == 0.0) {
          ...
          return new PatchedChiSqTestResult(0.0, Double.PositiveInfinity, Double.PositiveInfinity, size - 1, Double.PositiveInfinity, PEARSON.name,
    ...
    val df = size - 1
    val vCramer = sqrt(statistic.toDouble/nn/min(size, 2))
    val Phi2 = statistic.toDouble/nn
    val pValue = 1.0 - new ChiSquaredDistribution(df).cumulativeProbability(statistic)
    new PatchedChiSqTestResult(pValue, vCramer, Phi2,  df, statistic, PEARSON.name, NullHypothesis.goodnessOfFit.toString)
  }
   ...
   */
  def chiSquaredMatrix(counts: Matrix, methodName:String = PEARSON.name): PatchedChiSqTestResult = {
    val method = methodFromString(methodName)
    ...
    while (j < colMajorArr.size) {
      ...
      if (expected < 1) {
        logWarning(s"All individual expected counts shoud be 1 or greater")
      }
      statistic += method.chiSqFunc(colMajorArr(j), expected)
      j += 1
    }
    if (check.toDouble/(numCols * numRows)> 0.2) {
      logWarning(s"Chi-squared independence test may not be accurate due to low expected frequencies.")
    }
    ...
    if (df == 0) {
      ...
      val vCramer = 0.0
      val Phi2 = 0.0
      new PatchedChiSqTestResult(1.0, vCramer, Phi2, 0, 0.0, methodName, NullHypothesis.independence.toString)
    } else {
      val vCramer = sqrt(statistic.toDouble/total/min(numRows-1, numCols-1))
      val Phi2 = statistic.toDouble/total
      val pValue = 1.0 - new ChiSquaredDistribution(df).cumulativeProbability(statistic)
      new PatchedChiSqTestResult(pValue, vCramer, Phi2, df, statistic, methodName, NullHypothesis.independence.toString)
    }
  }
}
