...
object WilcoxonTest {
  ...
  def multiply(
                ...
                function: (Array[Double], Array[Double]) => Double,
    ...
    val topkSims = blockedBig.cartesian(blockedSmall).flatMap {
      case ((bigBlockIndex, bigRows), (smallBlockIndex, smallRows)) =>
        ...
        for (i <- 0 until bigRows.size; j <- 0 until smallRows.size if smallRows(j)._1 != bigRows(i)._1) {
          ...
          val testval = function(bigRow, smallRow)
          //val testval = 0.1
  ...
  def run(args: Array[String]): RDD[(Int, Int, Double)] = {
   ...
   // sc.addJar("lib/commons-math3-3.3.jar")
    ...
    val statres = multiply(data_col, data_col, WilcoxRankSumTest.math_compute, -1)
    val res_show = statres.takeSample(false, 50, 29)
    val end = System.currentTimeMillis / 1000

    println(res_show.map(i => "Random sample is: " + i._1 + "," + i._2 + "," + i._3).mkString("\n"))
    println("*********************************************************************************")
    println("*********************************************************************************")
